...
def self_get_0_1_d(d, new_max=1, new_min=0):
	d_min = np.min(d)
	d_max = np.max(d)
	d = ((d-d_min)/(d_max-d_min))*(new_max-new_min)+new_min
	return d

def self_draw_distance_hotmap(distance_vertex_line):

	plt.matshow(distance_vertex_line, cmap='autumn')
	plt.colorbar()
	plt.show()

def self_get_pixel(p, origin_img):
	try:
...
def self_nearest_neighbor_interpolation(xy, new_origin_img):
	# xy = np.around(xy_).astype(np.int)
	origin_pixel = self_get_pixel([xy[0], xy[1]], new_origin_img)
	if (origin_pixel == 256).all():
		return origin_pixel, False
	return origin_pixel, True

def self_bilinear_interpolation(xy_, new_origin_img):
	xy_int = [int(xy_[0]), int(xy_[1])]
	xy_decimal = [round(xy_[0] - xy_int[0], 5), round(xy_[1] - xy_int[1], 5)]
	x0_y0 = (1 - xy_decimal[0]) * (1 - xy_decimal[1]) * self_get_pixel([xy_int[0], xy_int[1]], new_origin_img)

	x0_y1 = (1 - xy_decimal[0]) * (xy_decimal[1]) * self_get_pixel([xy_int[0], xy_int[1] + 1], new_origin_img)

	x1_y0 = (xy_decimal[0]) * (1 - xy_decimal[1]) * self_get_pixel([xy_int[0] + 1, xy_int[1]], new_origin_img)

	x1_y1 = (xy_decimal[0]) * (xy_decimal[1]) * self_get_pixel([xy_int[0] + 1, xy_int[1] + 1], new_origin_img)

	return x0_y0, x0_y1, x1_y0, x1_y1

def self_get_coor(p, origin_label):
	try:
...
def self_bilinear_interpolation_coordinate_v4(xy_, new_origin_img):
	...
	x0, x1, x2, x3 = 0, 0, 0, 0
	y0, y1, y2, y3 = 0, 0, 0, 0
	x0_y0 = self_get_coor(np.array([xy_int[0], xy_int[1]]), new_origin_img)
	x0_y1 = self_get_coor(np.array([xy_int[0], xy_int[1]+1]), new_origin_img)
	x1_y0 = self_get_coor(np.array([xy_int[0]+1, xy_int[1]]), new_origin_img)
	x1_y1 = self_get_coor(np.array([xy_int[0]+1, xy_int[1]+1]), new_origin_img)
...
def self_is_perform(execution, inexecution):
	return random.choices([True, False], weights=[execution, inexecution])[0]

def self_get_margin_scale(min_, max_, clip_add_margin, new_shape):
...
def self_adjust_position(x_min, y_min, x_max, y_max,self_new_shape):
	if (self_new_shape[0] - (x_max - x_min)) % 2 == 0:
		f_g_0_0 = (self_new_shape[0] - (x_max - x_min)) // 2
		f_g_0_1 = f_g_0_0
	else:
		f_g_0_0 = (self_new_shape[0] - (x_max - x_min)) // 2
		f_g_0_1 = f_g_0_0 + 1

	if (self_new_shape[1] - (y_max - y_min)) % 2 == 0:
		f_g_1_0 = (self_new_shape[1] - (y_max - y_min)) // 2
		f_g_1_1 = f_g_1_0
	else:
		f_g_1_0 = (self_new_shape[1] - (y_max - y_min)) // 2
		f_g_1_1 = f_g_1_0 + 1
	...
	return f_g_0_0, f_g_1_0, self_new_shape[0] - f_g_0_1, self_new_shape[1] - f_g_1_1

def self_adjust_position_v2(x_min, y_min, x_max, y_max, new_shape):
...
def self_adjust_border(x_min, y_min, x_max, y_max, x_min_new, y_min_new, x_max_new, y_max_new):
...
def self_interp_weights(xyz, uvw):
...
def self_interpolate(values, vtx, wts):
	return np.einsum('njk,nj->nk', np.take(values, vtx, axis=0), wts)

def self_pad(synthesis_perturbed_img_map, x_min, y_min, x_max, y_max):
...
def self_isSavePerturbed(synthesis_perturbed_img, new_shape):
...
def self_get_angle(A, o, B):
	v1 = o-A
	v2 = o-B
	return np.arccos((v1 @ v2) / (np.linalg.norm(v1) * np.linalg.norm(v2)))*180/np.pi

def self_get_angle_4(pts):
	a0_ = self_get_angle(pts[2], pts[0], pts[1])
	a1_ = self_get_angle(pts[0], pts[1], pts[3])
	a2_ = self_get_angle(pts[3], pts[2], pts[0])
	a3_ = self_get_angle(pts[1], pts[3], pts[2])
